...
**Step 07 — Anomaly detection rules.** Operates on `OperationalEvent`. Sliding-window threshold rules over Redis counters, emits anomalies.

Housekeeping before step 07 (small):
- Rewrite `step-06-kafka.md` — currently describes the pre-hand-written approach (Spring Kafka + `@KafkaListener`), needs to reflect actual implementation (`AbstractKafkaConsumer`, `KafkaTopicInitializer`, `INGESTION_FAILED` failure sink, config-driven timings and retry)
- Remove `mockwebserver` dep from payment-service pom (unused since 06c4)
- Remove `SENTINEL_URL` env var from compose.yml (unused since 06c4)
- e2e verify the 06e failure path (publish malformed JSON manually via `kafka-console-producer`, confirm `INGESTION_FAILED` event lands in `operational_events`)
